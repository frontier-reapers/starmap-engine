...
fn reconstruct_path(graph: &StarGraph, came_from: &HashMap<usize, usize>, mut current: usize) -> Vec<PathStep> {
...
mod tests {
    ...
    fn simple_triangle_path() {
        ...
        let ids: Vec<u32> = path.iter().map(|p| graph.systems[p.system_index].id).collect();
